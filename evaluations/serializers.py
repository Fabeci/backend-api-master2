...
class EvaluationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Evaluation
        fields = [
            'id', 'cours', 'cours_titre', 'enseignant', 'enseignant_nom',
            'titre', 'type_evaluation', 'type_evaluation_display', 'bareme',
            'duree_minutes', 'consigne_texte', 'fichier_sujet', 'date_debut',
            'date_fin', 'date_creation', 'date_modification', 'est_publiee',
            'nombre_questions', 'est_accessible', 'peut_soumettre',
            'est_auto_corrigeable',
        ]
        read_only_fields = [
            'id', 'date_creation', 'date_modification', 'nombre_questions',
            'est_accessible', 'peut_soumettre', 'est_auto_corrigeable',
        ]
    ...
    def create(self, validated_data):
        validated_data.pop('questions_ids', None)  # ignorer si envoyé
...
class PassageEvaluationDetailSerializer(PassageEvaluationSerializer):
    ...
    evaluation_detail = serializers.SerializerMethodField()   # ← renommé sans "s"

    class Meta(PassageEvaluationSerializer.Meta):
        fields = PassageEvaluationSerializer.Meta.fields + [
            'reponses_questions',
            'evaluation_detail',     # ← renommé sans "s"
        ]

    def get_evaluation_detail(self, obj):
        """Retourne les détails complets de l'évaluation AVEC ses questions"""
        from .serializers import QuestionSerializer  # import local pour éviter circular

        questions = obj.evaluation.questions.prefetch_related(
            'reponses_predefinies'
        ).all()
        ...
        return {
            ...
            'fichier_sujet': (
                obj.evaluation.fichier_sujet.url
                if obj.evaluation.fichier_sujet else None
            ),
            ...
            # ✅ AJOUT CRITIQUE : les questions avec leurs réponses prédéfinies
